...
- 색상 강조가 과제 보고서보다 웹 랜딩 페이지처럼 보인다는 피드백을 반영해 본문 스타일을 흑백 중심으로 낮췄다.
- V-Model을 문장으로만 설명하지 않고, 요구사항 분석과 인수 기준, 설계와 시스템 검증, 구현과 자동 테스트가 대응되는 도식으로 추가했다.
- 반복 개발 프로세스와 화면 렌더링 흐름도 각각 프로세스 다이어그램과 시퀀스 다이어그램으로 본문에 포함했다.
