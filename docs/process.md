...
`npm test`는 JavaScript 문법 검사, 데이터 구조 검증, UI 계약 검증, JSON 파싱 검증을
한 번에 수행한다. 같은 명령을 GitHub Actions에서도 실행하여 로컬 검증과 원격 검증의
기준을 맞춘다.
...
| Phase 2 지도/위험지수 | Leaflet 지도, 지표 전환, 지역 선택, 시나리오, 지역 비교, 산식 설명 구현 완료 |
| Phase 3 분석 기능 | 지역 탐색, 대응 전략, 정책 대응 시뮬레이터, 폐교 통계 분석, 데이터 신뢰도 패널 구현 완료 |
| 보고서 지원 기능 | 보고서 요약 Markdown 내보내기, 캡처 모드, 인쇄 스타일, PWA 설치성 구현 완료 |
| 프로세스 문서 | 요구사항, 설계 결정, 테스트 계획, lessons learned, AI 로그 작성 및 갱신 |
| 자동 검증 | `npm test`, GitHub Actions CI, Pages 배포 전 검증 구성 |
| 제출 증빙 | `docs/submission-evidence.md`와 2026-06-02 화면 캡처 추가 |
| 남은 과제 | 최종 PDF 변환, 모바일 캡처, 공식 시도별 데이터 보강 |

다음 단계는 최종 보고서 PDF화와 모바일/캡처 증빙 보강이다. 데이터 측면에서는 요약 지표는
공식 통계로 교체했지만, 시도별 분석값은 아직 임시 데이터가 남아 있으므로 최종 보고서에서
한계를 명시하거나 후속 커밋에서 단계적으로 교체한다.
